...
lfs::tcp::ResponderServer::~ResponderServer() {
    ResponderServer::stop();
}

void lfs::tcp::ResponderServer::start() {
    running_ = true;
    response_thread_ = std::thread(&ResponderServer::run, this);
}

void lfs::tcp::ResponderServer::stop() {
    running_ = false;
...
void lfs::tcp::ResponderServer::join() {
    if (response_thread_.joinable()) {
        response_thread_.join();
    }
}

void lfs::tcp::ResponderServer::run() {
...
nlohmann::json lfs::tcp::ResponderServer::generateResponse(const nlohmann::json& request) {
...
nlohmann::json lfs::tcp::ResponderServer::getValue(std::string_view parameter, bool& success) {
    success = true;
    if (parameter == "state") {
