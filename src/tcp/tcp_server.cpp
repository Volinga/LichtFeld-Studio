...
void lfs::tcp::TCPServer::stop() {
    ;
}

void lfs::tcp::TCPServer::join() {
    ;
}

std::string lfs::tcp::TCPServer::getEndpoint() const {
    return socket_.get(zmq::sockopt::last_endpoint);
}

void lfs::tcp::TCPServer::send(const nlohmann::json& data) {
    socket_.send(toZMQ(data), zmq::send_flags::none);
}

bool lfs::tcp::TCPServer::receive(nlohmann::json& data) {
...
zmq::message_t lfs::tcp::TCPServer::toZMQ(const nlohmann::json& data) {
...
nlohmann::json lfs::tcp::TCPServer::fromZMQ(const zmq::message_t& msg_zmq, unsigned long long size) {
